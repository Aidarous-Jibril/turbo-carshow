...
import { yearsOfProduction, manufacturers } from '@/constants';
...
const Form = ({ carInfo, setCarInfo, submitBtnTitle, title, handleSubmit, isLoading,  }: FormProps) => {
    ...
    const [accepetedImages, setAccepetedImages] = useState<File[]>([]);
    ...
    useEffect(() => {
        ...
        accepetedImages?.map((file) => convertBase64(file));

    }, [accepetedImages, carInfo]);
            ...
            <div className='flex flex-col gap-6 md:gap-7'>
                ...
                <div className='flex flex-col md:flex-row items-center w-full gap-1 md:gap-4'>
                    ...
                    <div className='w-full'>
                        ...
                        <CustomSelect label='Car Type' onChange={(value, name) => handleSelectChange(value, name)} options={[{ title: 'Toyota', value: 'toyota' }, { title: 'Mercedes', value: 'mercedes' }, { title: 'Volvo', value: 'volvo' }, { title: 'Kia', value: 'kia' }, { title: 'BMW', value: 'bmw' }]} containerStyle='border dark:border-slate-700 rounded-md py-3' name='carType' />
                    </div>
                    <div className='w-full'>
                ...
                {/* custom components */}
                <ImgUploader files={accepetedImages} handleDragDrop={handleDragDrop} carInfo={carInfo} />
                <CustomBtn title={submitBtnTitle} btnType='submit' optionStyles="bg-blue-500 hover:bg-blue-700 text-white font-bold justify-center items-center mt-4 py-2 px-4 rounded-full w-fit  " />
            </div>
        </form>
    )
}
